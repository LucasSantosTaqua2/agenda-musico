...
// --- MUDANÇA DE ABORDAGEM: Gerenciamento Centralizado de Views ---
const allViews = {
    agenda: document.getElementById('view-agenda'),
    calendario: document.getElementById('view-calendario'),
    setlists: document.getElementById('view-setlists'),
    equipe: document.getElementById('view-equipe'),
    dashboard: document.getElementById('view-dashboard'),
    stagePlot: document.getElementById('view-stage-plot'),
    rider: document.getElementById('view-rider'),
...
const allTabs = {
    ...
    setlists: document.getElementById('tab-setlists'),
    equipe: document.getElementById('tab-equipe'),
    dashboard: document.getElementById('tab-dashboard'),
    stagePlot: document.getElementById('tab-stage-plot'),
    rider: document.getElementById('tab-rider'),
...
const switchView = (viewToShow) => {
    ...
    // Mostra a view e ativa a aba correspondente
    if (allViews[viewToShow] && allTabs[viewToShow]) {
        allViews[viewToShow].classList.remove('hidden');
        allTabs[viewToShow].classList.add('tab-active');
        lucide.createIcons();

        // Lógica específica para o dashboard (re-renderiza o mapa)
...
// Adiciona os event listeners para cada aba usando o objeto
Object.keys(allTabs).forEach(tabKey => {
    const viewKey = tabKey.replace('tab-', 'view-'); // Converte 'tab-agenda' para 'agenda'
    if (allTabs[tabKey]) {
        allTabs[tabKey].addEventListener('click', () => switchView(viewKey));
    }
});
// --- FIM DA MUDANÇA DE ABORDAGEM ---
...
// ... (todas as outras funções do main.js, como renderColorPickers, openEditShowModal, etc., permanecem as mesmas. Copie e cole o resto do seu main.js aqui, a partir da função renderColorPickers até o final do arquivo) ...
...
const renderColorPickers = colorSettings => {
  ...
  const openShowDetailsModal = showId => {
    ...
    loadAndRenderComments(show.ownerId, show.id);
    ...
    addCommentForm.addEventListener('submit', e => {
      ...
      if (commentText) {
        addComment(show.ownerId, show.id, commentText);
        commentInput.value = '';
      }
    });
  ...
  confirmDeleteBtn.addEventListener('click', async () => {
    ...
    } else if (itemTypeToDelete === 'rider') { 
  ...
  const renderSetlists = () => {
    ...
    allSetlists.forEach(setlist => {
      ...
      setlistElement.className = 'bg-gray-800 rounded-lg shadow-md'; 
  ...
  document.getElementById('export-stage-plot-btn').addEventListener('click', () => {
      const stageAreaContainer = document.getElementById('stage-area-container');
      const placeholder = stageArea.querySelector('p');
      ...
      html2canvas(stageAreaContainer, {
